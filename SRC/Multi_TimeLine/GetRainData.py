...
def retrieve_rain_gauge_data(ID):
    ...
    file.write('aaa=$(wget "http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av?p_nccObsCode=136&p_display_type=dailyDataFile&p_startYear=&p_c=&p_stn_num='+format(ID, '06d')+'" -O log -U "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36"; grep "All years" log | awk \'{print $25}\' - | awk \'BEGIN{FS="\\\""} {print $2}\' - | sed "s/amp;//g" -)\nrm -f log\nbbb=$(echo -n "http://www.bom.gov.au"$aaa)\nwget $bbb -U "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36" -O temp.zip\nunzip temp.zip -x *Note*\nrm -f temp.zip\n')
    file.close()
...
def get_rain_data(lat, lon):
    ...
    #
    df = pd.read_csv('/g/data/up71/projects/CalVal_Phase1/Weather/ShortStations.txt')
    ...
    for i in range(len(df1.index)):
        ...
        if float(rain_gauge[rain_gauge.Year >= 2013]['Rainfall amount (millimetres)'].count())/float(DaysSince2013) > 0.9:
            ...
            os.system('mv -f IDCJAC0009_'+format(ID, '06d')+'_1800_Data.csv /g/data/up71/projects/CalVal_Phase1/Weather')
            break
        else:

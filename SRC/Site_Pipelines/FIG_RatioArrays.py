...
def FIG_ratio_arrays(sat_array, field_array, output, field_data, fignum):
    ...
    fig, axes = plt.subplots(nrows=4, ncols=3, figsize=(12.5, 10.5))
    #fig.suptitle(fig_title+': Ratio of satellite/field reflectance', fontweight='bold')
    plt.tight_layout(pad=3.5, w_pad=4.5, h_pad=2.5)
    ...
    if field_data[3] == 'Landsat8':
        ...
        newarr.nbart_nir.plot(ax=axes[1,1], levels=levels)
        newarr.nbart_swir_1.plot(ax=axes[1,2], levels=levels)
        newarr.nbart_swir_2.plot(ax=axes[2,0], levels=levels)
        axes[2,1].axis('off')
        axes[2,2].axis('off')
        axes[3,0].axis('off')
        axes[3,1].axis('off')

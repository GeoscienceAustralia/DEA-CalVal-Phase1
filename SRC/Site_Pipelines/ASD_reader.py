...
def parse_metadata(asd):
    ...
    application, channels, app_data, gps_data, integration_time, fo, dcc, calibration, instrument_num, \
...
def parse_calibration_header(asd, offset):
    ...
    #print("plop1", len(asd), offset, len(asd) - offset, struct.calcsize(header_format), buffer_count)
    offset += struct.calcsize(header_format)

    calibration_buffer = []
    for i in range(buffer_count):
        (cal_type, name, integration_time, swir1gain, swir2gain) = struct.unpack_from(calibration_buffer_format, asd, offset)
        name = name.strip(b'\x00')

        calibration_buffer.append(((cal_type, name, integration_time, swir1gain, swir2gain)))
        offset += struct.calcsize(calibration_buffer_format)

    #print("plop2", calibration_buffer)
...
def normalise_spectrum(spec, metadata):
    ...
    res[:splice1_index] = spec[:splice1_index] / metadata.integration_time
...
class reader:
    ...
    def get_radiance(self):
        if spectra_type[self.md.data_type] == 'RAD':
            res = self.calibration_lamp * self.reference * self.spec * self.md.integration_time / \

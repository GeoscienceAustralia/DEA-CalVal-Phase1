...
def SDev(sat_array, satname):
    ...
    for k in sat_array.data_vars:
        if len(sat_array[k].x)+len(sat_array[k].y) > 2*satfieldpix:
            loost = []
            for i in range(len(sat_array[k].x)-int(satfieldpix)-1):
                for j in range(len(sat_array[k].y)-int(satfieldpix)-1):
                    loost.append(float(sat_array[k][0][i:i+int(satfieldpix), j:j+int(satfieldpix)].mean()))

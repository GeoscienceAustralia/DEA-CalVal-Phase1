...
#
def FIG_BRDF_comp(sat_array, fstat_df, finner_df, finner_sbrdf_df, output, field_data, fignum):

    fig_title = 'Figure '+str(fignum)+': '+field_data[0]+' '+field_data[1]+' '+field_data[2]+' '+field_data[3]
    fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(9.5, 9.5))
    fig.suptitle(fig_title+': Satellite and Field data (2 different BRDFs) comparison by band for inner pixels', fontweight='bold')
    ...
    finner_sbrdf_df.plot(y='Field_inner_mean', ax=axes, color='orange')
